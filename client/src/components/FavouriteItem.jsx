...
const FavoriteItem = ({ favorite, listing, onRemoveFavorite }) => {
  ...
  const handleToggleFavourite = (listingId, isFavourite) => {
    if (isFavourite) {
      ...
      // Call the onRemoveFavorite callback to remove from favorites
      onRemoveFavorite(listingId);
    }
  };

  // Render FavoriteItem component
  return (
    <div
      className="
    bg-white 
    ...
    >
      ...
      <div className="p-3 flex flex-col gap-2 w-full">
        ...
        <p className="truncate text-lg font-semibold text-slate-700">
          {/* Link to the detailed listing page */}
          <Link to={`/listing/${favorite._id}`}>{favorite.name}</Link>
        </p>
        {/* Location */}
        <div className="flex items-center gap-1">
          ...
          <p className="text-sm text-gray-600 truncate w-full">
            {/* Link to the detailed listing page */}
            <Link to={`/listing/${favorite._id}`}>{favorite.address}</Link>
          </p>
        </div>
        {/* Description */}
        <p className="text-sm text-gray-600 line-clamp-2">
          {/* Link to the detailed listing page */}
          <Link to={`/listing/${favorite._id}`}>{favorite.description}</Link>
        </p>
        {/* Price */}
        <p className="text-slate-500 mt-2 font-semibold">
          Rs {favorite.regularPrice.toLocaleString("en-US")}
          {favorite.type === "rent" && " / month"}
        </p>
        ...
        <div
          className="flex 
            ...
            rounded-md "
        >
          {/* Heart component for toggling favorite status */}
          <Heart
